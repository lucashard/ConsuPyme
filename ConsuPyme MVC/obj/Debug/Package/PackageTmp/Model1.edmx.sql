...
-- --------------------------------------------------
-- Date Created: 04/16/2016 01:35:18
-- Generated from EDMX file: C:\01 ARCHIVOS ESCRITORIO\Posta1\ConsuPyme MVC\Model1.edmx
-- --------------------------------------------------
...
GO
IF OBJECT_ID(N'[dbo].[FK_DepositoDespacho]', 'F') IS NOT NULL
    ALTER TABLE [dbo].[Deposito] DROP CONSTRAINT [FK_DepositoDespacho];
GO
IF OBJECT_ID(N'[dbo].[FK_AcarreoDespacho]', 'F') IS NOT NULL
    ALTER TABLE [dbo].[Acarreo] DROP CONSTRAINT [FK_AcarreoDespacho];
GO
IF OBJECT_ID(N'[dbo].[FK_DespachoTipo_Cambio]', 'F') IS NOT NULL
    ALTER TABLE [dbo].[Despacho] DROP CONSTRAINT [FK_DespachoTipo_Cambio];
GO
IF OBJECT_ID(N'[dbo].[FK_ProveedorFactura_Total]', 'F') IS NOT NULL
    ALTER TABLE [dbo].[Factura_Total] DROP CONSTRAINT [FK_ProveedorFactura_Total];
GO
...
GO
IF OBJECT_ID(N'[dbo].[Tipo_Cambio]', 'U') IS NOT NULL
...
CREATE TABLE [dbo].[Despachante] (
    ...
    [Nombre] nvarchar(max)  NOT NULL,
    [Numero_Factura] nvarchar(max)  NOT NULL,
    [Ley] decimal(18,2)  NOT NULL,
    [Djai] decimal(18,2)  NOT NULL,
    [AD_Sim] decimal(18,2)  NOT NULL,
    [Gastos_Despacho] decimal(18,2)  NOT NULL,
    [Desconsolidado] decimal(18,2)  NOT NULL,
    [Servicios] decimal(18,2)  NOT NULL,
    [DespachoId] int  NOT NULL,
    [Fecha] datetime  NOT NULL,
    [Gestion_Urgente] decimal(18,2)  NOT NULL,
    [Federal_Express] decimal(18,2)  NOT NULL
);
GO
...
CREATE TABLE [dbo].[Despacho] (
    ...
    [Fob_Total] decimal(18,2)  NOT NULL,
    [Flete_Total] decimal(18,2)  NOT NULL,
    [Seguro_Total] decimal(18,2)  NOT NULL,
    [Cotizacion] decimal(18,2)  NOT NULL,
    [Codigo_Afip] nvarchar(max)  NOT NULL,
    [Arancel_Sim] decimal(18,2)  NOT NULL,
    [Servicio_Guarda] decimal(18,2)  NOT NULL,
    [Tipo_CambioId] int  NOT NULL,
    [Gasto_Aduana] decimal(18,2)  NOT NULL,
    [Porcentaje_Fob] decimal(18,2)  NOT NULL,
    [Derechos] decimal(18,2)  NOT NULL,
    [Taza_Estadistica] decimal(18,2)  NOT NULL,
...
CREATE TABLE [dbo].[Posicion_Arancelaria] (
    ...
    [Porcentaje_Taza_Estadistica] decimal(18,2)  NOT NULL,
    [Porcentaje_Importacion] decimal(18,2)  NOT NULL
);
GO
...
-- Creating primary key on [Id] in table 'Tipo_Cambio'
ALTER TABLE [dbo].[Tipo_Cambio]
ADD CONSTRAINT [PK_Tipo_Cambio]
    PRIMARY KEY CLUSTERED ([Id] ASC);
GO
...
-- Creating foreign key on [Tipo_CambioId] in table 'Despacho'
ALTER TABLE [dbo].[Despacho]
ADD CONSTRAINT [FK_DespachoTipo_Cambio]
    FOREIGN KEY ([Tipo_CambioId])
    REFERENCES [dbo].[Tipo_Cambio]
        ([Id])
    ON DELETE NO ACTION ON UPDATE NO ACTION;

-- Creating non-clustered index for FOREIGN KEY 'FK_DespachoTipo_Cambio'
CREATE INDEX [IX_FK_DespachoTipo_Cambio]
ON [dbo].[Despacho]
    ([Tipo_CambioId]);
GO

-- Creating foreign key on [ProveedorId] in table 'Factura_Total'
ALTER TABLE [dbo].[Factura_Total]
ADD CONSTRAINT [FK_ProveedorFactura_Total]
    FOREIGN KEY ([ProveedorId])
    REFERENCES [dbo].[Proveedor]
        ([Id])
    ON DELETE NO ACTION ON UPDATE NO ACTION;

-- Creating non-clustered index for FOREIGN KEY 'FK_ProveedorFactura_Total'
CREATE INDEX [IX_FK_ProveedorFactura_Total]
ON [dbo].[Factura_Total]
    ([ProveedorId]);
GO
